...
- P1.6 TLS: Kestrel serves HTTPS with a certificate from the vault, or `ForwardedHeaders` is restricted to `KnownProxies`/`KnownNetworks`. `UseHsts()`. In `Production`, refuse to start when neither is configured.
...
- P6.4 TLS terminated at the ingress with a trusted-proxy configuration, or in Kestrel; `AllowedHosts` set to the public name; HSTS enabled; mutual TLS between ingress and pod where the platform supports it.
...
| SPEC-06 | TLS or trusted proxy required in Production | Startup check | Production without TLS → exit 78 | 1, 6 |
